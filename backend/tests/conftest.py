...
def default_mock_current_user(request):
    ...
    """
    # If the test is in test_auth.py, test_properties_my.py, test_favorites.py, test_agent.py, or test_alerts, do not override globally to allow testing 401 unauth
    if any(k in request.node.nodeid for k in ["test_auth", "test_properties_my", "test_favorites", "test_agent", "test_alerts"]):
        yield
        return
